...
namespace BlazorServerDemo.Tests
{
    public class InMemoryStudentRepositoryTests
    {
        ...
        {
            var repo = NewRepo();
            var s = await repo.AddAsync(new Student { FirstName = "Katherine", LastName = "Johnson", Email = "kj@uni.edu", Year = 1 });
        ...
        {
            var repo = NewRepo();
            var s = new Student { Id = Guid.NewGuid(), FirstName = "Dup", LastName = "One", Email = "dup1@uni.edu", Year = 1 };
            await repo.AddAsync(s);


            var duplicate = new Student { Id = s.Id, FirstName = "Dup", LastName = "Two", Email = "dup2@uni.edu", Year = 2 };
